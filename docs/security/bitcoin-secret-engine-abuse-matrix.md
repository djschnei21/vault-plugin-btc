...
| Missing PSBT UTXO or redeem-script context | `sign`, `sign-raw` | Rejected fail-closed before signing | `tests/security_sign_policy.rs` (`sign_rejects_psbt_without_utxo_script_context`, `sign_rejects_nested_segwit_without_redeem_script_context`, `sign_rejects_nested_segwit_with_mismatched_redeem_script`) | Continue broader sign policy review outside the approved remediation scope |
