...
fn psbt_base64_with_nested_segwit_input_mismatched_redeem_script(
    ...
    psbt.inputs[0].redeem_script = Some(ScriptBuf::new_p2wpkh(
        &bitcoin::WPubkeyHash::from_byte_array([7; 20]),
    ));
...
#[tokio::test]
async fn sign_rejects_nested_segwit_with_mismatched_redeem_script() {
    let router = Router::new();
    let storage = Arc::new(InMemoryStorage::new());
    ...
    let err = router
        .route(
            &request(
                ...
                "wallets/policy-wallet/sign",
                Some(serde_json::json!({
                    "psbt": psbt_base64_with_nested_segwit_input_mismatched_redeem_script(address.script_pubkey()),
                })),
            ),
            storage,
        )
        .await
        .expect_err("nested segwit signing must reject mismatched redeem script binding");
